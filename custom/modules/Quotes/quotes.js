...
function QuotesEditManager(Y) {
    ...
    this.openPopup = function(value, count) {
        var popup_request_data = {"call_back_function": "set_product_return", "form_name": "EditView", "field_to_name_array": {"id": "id", "name": "name", "cost_usdollar": "cost_usdollar", "list_usdollar": "list_usdollar", "discount_usdollar": "discount_usdollar", "mft_part_num": "mft_part_num", "pricing_factor": "pricing_factor", "type_id": "type_id", "tax_class": "tax_class", "tax_class_name": "tax_class_name", "description": "description", "litem_selection_c": "litem_selection_c" }, "passthru_data": {"row_id": count}};
    ...
    this.set_value_line_item_selection = function(doc, count) {
        if (doc.getElementById('checkbox_select_' + count).checked == true) {
            $('#litem_selection_c_'+ count).val(true);
        }
        else {
            $('#litem_selection_c_'+ count).val(false);
        }
        //console.log(doc.getElementById('litem_selection_c_' + count).id+' value='+  $('#litem_selection_c_'+ count).val());
    }
    ...
    this.addTable = function(id, bundle_stage, bundle_name, bundle_shipping) {
        ...
        var textEl = this.createElement('input', {'type': 'hidden', 'name': "delete_table[" + id + "]", 'id': "delete_table_" + id, 'value': 1});
        item_list_MSI["delete_table[" + id + "]"] = textEl;
        form.appendChild(textEl);
        var tables = Y.one('#add_tables')._node;
        var tableEl = this.createElement('table', {'name': id, 'id': id, 'border': 0, 'cellspacing': 1, 'cellpadding': 0});
        item_list_MSI[id] = tableEl;
        var newHR = this.createElement('hr', {'id': id + '_hr1'});
        var newDIV = this.createElement('div', {'id': id + '_header'});
        ...
        var newHR = this.createElement('hr', {'id': id + '_hr2'});
        tables.appendChild(newHR);
        var tableEl = this.createElement('table', {'name': id + '_tally', 'id': id + '_tally', 'border': 0, 'cellspacing': 1, 'cellpadding': 0});
    ...
    this.addTableTally = function(id) {
        ...
        var inputEl = this.createElement('input', {'type': 'button', 'name': this.addRowName, 'value': this.addRowValue}, ['button']);
        inputEl.tableId = id;
        inputEl.onclick = function() {
            quotesManager.addRow("", "", "", "", 0, 0, "", "", "", "", "", "", "", this.tableId, '', '', '', '', '', '0', '', '', '',1);
        }
        tdEl.appendChild(inputEl);
        var inputEl = this.createElement('input', {'type': 'button', 'name': this.addCommentName, 'value': this.addCommentValue}, ['button']);
        inputEl.tableId = id;
        inputEl.onclick = function() {
        ...
        tdEl.width = 70;
        rowEl.appendChild(tdEl);
        var inputEl = this.createElement('input', {'type': 'text', 'size': 15, 'name': 'subtotal[' + id + ']', 'id': 'subtotal_' + id});
        ...
        rowEl.appendChild(tdEl);
        var inputEl = this.createElement('input', {'type': 'hidden', 'size': 15, 'name': 'subtotal_usdollar[' + id + ']', 'id': 'subtotal_usdollar' + id});
        inputEl.readOnly = true;
        tdEl.appendChild(inputEl);
        inputEl.style.textAlign = 'right';
        ...
        tdEl.width = 70;
        rowEl.appendChild(tdEl);
        var inputEl = this.createElement('input', {'type': 'text', 'size': 15, 'name': 'deal_tot[' + id + ']', 'id': 'deal_tot_' + id});
        ...
        tdEl.width = 70;
        rowEl.appendChild(tdEl);
        var inputEl = this.createElement('input', {'type': 'text', 'size': 15, 'name': 'new_sub[' + id + ']', 'id': 'new_sub_' + id});
        inputEl.onchange = function() {
        ...
        tdEl.width = 70;
        rowEl.appendChild(tdEl);
        var inputEl = this.createElement('input', {'type': 'text', 'name': 'tax[' + id + ']', 'id': 'tax_' + id, 'size': 15});
        inputEl.readOnly = true;
        inputEl.style.textAlign = 'right';
        this.setToReadOnly(inputEl);
        tdEl.appendChild(inputEl);
        ...
        rowEl.appendChild(tdEl);
        var inputEl = this.createElement('input', {'type': 'hidden', 'name': 'tax_usdollar[' + id + ']', 'id': 'tax_usdollar_' + id});
        inputEl.readOnly = true;
        inputEl.style.textAlign = 'right';
        tdEl.appendChild(inputEl);
        rowEl.appendChild(tdEl);
        ...
        tdEl.width = 70;
        rowEl.appendChild(tdEl);
        var inputEl = this.createElement('input', {'type': 'text', 'name': 'shipping[' + id + ']', 'id': 'shipping_' + id, 'size': 15});
        inputEl.onchange = function() {
            if (quotesManager.isAmount(toDecimal(unformatNumber(this.value, num_grp_sep, dec_sep), quotesManager.precision))) {
                quotesManager.calculate(document);
            } else {
        ...
        tdEl.appendChild(inputEl);
        rowEl.appendChild(tdEl);
        var inputEl = this.createElement('input', {'type': 'hidden', 'name': 'shipping_usdollar[' + id + ']', 'id': 'shipping_usdollar_' + id});
        inputEl.readOnly = true;
        inputEl.style.textAlign = 'right';
        tdEl.appendChild(inputEl);
        rowEl.appendChild(tdEl);
        ...
        tdEl.width = 550;
        rowEl.appendChild(tdEl);
        var inputEl = this.createElement('input', {'type': 'button', 'name': this.deleteTableName, 'value': this.deleteTableValue}, ['button']);
        inputEl.tableId = id;
        inputEl.onclick = function() {
        ...
        var tdEl = this.blankDataLabel.cloneNode(false);
        tdEl.width = 70;
        var inputEl = this.createElement('input', {'type': 'text', 'name': 'total[' + id + ']', 'id': 'total_' + id, 'size': 15});
        inputEl.readOnly = true;
        inputEl.style.textAlign = 'right';
        this.setToReadOnly(inputEl);
        tdEl.appendChild(inputEl);
        ...
        var tdEl = this.blankDataLabel.cloneNode(false);
        tdEl.width = 70;
        var inputEl = this.createElement('input', {'type': 'hidden', 'name': 'total_usdollar[' + id + ']', 'id': 'total_usdollar_' + id});
        inputEl.style.textAlign = 'right';
        inputEl.readOnly = true;
        tdEl.appendChild(inputEl);
        rowEl.appendChild(tdEl);
    ...
    this.addTableHeader = function(id) {
        ...
        tdEl.width = 75;
        tdEl.appendChild(document.createTextNode('Line Item Selection'));
        rowEl.appendChild(tdEl);
         var tdEl = this.blankDataLabel.cloneNode(false);
    ...
    this.addCommentRow = function(id, table_id, comment_description) {
        ...
        cell.nowrap = 'nowrap';
        var buttonEl = this.createElement('input', {'type': 'button', 'name': '||', 'value': '||'}, ['button']);
        ...
        var cell = row.insertCell(row.cells.length);
        cell.colSpan = "8";
        var textEl = this.createElement('input', {'type': 'hidden', 'name': "comment_id[" + this.count + "]", 'id': "comment_id_" + this.count, 'value': id});
        form.appendChild(textEl);
        comment_description = comment_description.replace(/&#039;/g, '\'');
        var textEl = this.createElement('textarea', {'rows': 3, 'cols': 120, 'name': "comment_description[" + this.count + "]", 'value': comment_description.replace(/<br>/g, '\n')});
        textEl.count = this.count;
        cell.appendChild(textEl);
        var textEl = this.createElement('input', {'type': 'hidden', 'name': "comment_index[" + this.count + "]", 'id': "comment_index" + this.count, 'value': this.count});
        cell.appendChild(textEl);
        var textEl = this.createElement('input', {'type': 'hidden', 'name': "comment_delete[" + this.count + "]", 'id': "comment_delete_" + this.count, 'value': 1});
        form.appendChild(textEl);
        var textEl = this.createElement('input', {'type': 'hidden', 'name': "parent_group[" + this.count + "]", 'id': "parent_group_" + this.count, 'value': table_id});
        form.appendChild(textEl);
        var textEl = this.createElement('input', {'type': 'hidden', 'name': "parent_group_index[" + this.count + "]", 'id': "parent_group_index" + this.count, 'value': row.rowIndex});
        item_list_MSI["parent_group_index_" + this.count] = textEl;
        cell.appendChild(textEl);
        var textEl = this.createElement('input', {'type': 'hidden', 'name': "parent_group_position[" + this.count + "]", 'id': "parent_group_position" + this.count, 'value': this.count});
        item_list_MSI["parent_group_position_" + this.count] = textEl;
        cell.appendChild(textEl);
        var cell = row.insertCell(row.cells.length);
        var buttonEl = this.createElement('input', {'type': 'button', 'name': this.deleteCommentName, 'id': 'delete_comment' + this.count, 'value': this.deleteCommentValue}, ['button']);
    ...
    this.addRow = function(id, quantity, product_template_id, product_name, cost_price, list_price, discount_price, pricing_formula, pricing_formula_name, pricing_factor, tax_class, tax_class_name, mft_part_num, table_id, bundle_stage, bundle_name, bundle_shipping, product_description, type_id, discount_amount, discount_select, deal_calc, product_status,litem_selection_c)
    {
        ...
        var unit_price;
        if (discount_price == '')
            unit_price = '0' + dec_sep + '00';
        else
            unit_price = discount_price;
        if (litem_selection_c == '')
            litem_selection_c = false;
        ...
        if (deal_calc == '')
            deal_calc = 0;
        if (product_status == '')
            product_status = this.default_product_status;
        var form = document.getElementById('EditView')
        var table = document.getElementById(table_id);
        var row = table.insertRow(table.rows.length);
        ...
        sqs_objects[sqs_id] = {"id": sqs_id, "form": form.id, "method": "query", "modules": ["ProductTemplates"], "group": "or", "field_list": ["name", "id", "type_id", "mft_part_num", "cost_price", "list_price", "discount_price", "tax_class", "pricing_factor", "description", "cost_usdollar", "list_usdollar", "discount_usdollar", "tax_class_name","litem_selection_c"], "populate_list": ["name_" + this.count, "product_template_id_" + this.count], "conditions": [{"name": "name", "op": "like_custom", "end": "%", "value": ""}], "order": "name", "limit": "30", "no_match_text": sqs_no_match_text, "post_onblur_function": "set_after_sqs"};
        row.setAttribute('valign', 'top');
        row.id = rowName;
        row.tableId = table.id;
        row.count = this.count;
        ...
        var textEl = this.createElement('input', {'type': 'checkbox', 'name': "checkbox_select[" + this.count + "]", 'id': itemName, 'value': '1'}, ['checkbox']);
        textEl.count = this.count;
        textEl.onclick = function() {
            quotesManager.set_value_line_item_selection(document, this.count);
        }
        if (litem_selection_c==true) {
            textEl.setAttribute('checked', true);
        }
        item_list_MSI[itemName] = textEl;
         cell11.appendChild(textEl);
        var itemName = 'litem_selection_c_' + this.count;
        var textEl = this.createElement('input', {'type': 'hidden', 'name': "litem_selection_c[" + this.count + "]", 'id': itemName, 'value': litem_selection_c});
        textEl.count = this.count;
        item_list_MSI[itemName] = textEl;                      
        cell11.appendChild(textEl);
        console.log('litem_selection_c='+litem_selection_c);           
        /////////////
        
        var cell = row.insertCell(row.cells.length);
        cell.nowrap = 'nowrap';
        var textEl = this.createElement('input', {'type': 'hidden', 'name': "product_id[" + this.count + "]", 'id': "product_id_" + this.count, 'value': id});
        item_list_MSI["product_id_[" + this.count + "]"] = textEl;
        form.appendChild(textEl);
        var textEl = this.createElement('input', {'type': 'hidden', 'name': "delete[" + this.count + "]", 'id': "delete_" + this.count, 'value': 1});
        item_list_MSI["delete[" + this.count + "]"] = textEl;
        form.appendChild(textEl);
        var textEl = this.createElement('input', {'type': 'hidden', 'name': "type_id[" + this.count + "]", 'id': "type_id_" + this.count, 'value': type_id});
        item_list_MSI["type_id[" + this.count + "]"] = textEl;
        form.appendChild(textEl);
        var textEl = this.createElement('input', {'type': 'hidden', 'name': "product_template_id[" + this.count + "]", 'id': "product_template_id_" + this.count, 'value': product_template_id});
        item_list_MSI["product_template_id[" + this.count + "]"] = textEl;
        form.appendChild(textEl);
        var textEl = this.createElement('input', {'type': 'hidden', 'name': "status[" + this.count + "]", 'id': "status[" + this.count + "]", 'value': product_status});
        item_list_MSI["status[" + this.count + "]"] = textEl;
        form.appendChild(textEl);
        var textEl = this.createElement('input', {'type': 'hidden', 'name': "tax_class[" + this.count + "]", 'id': "tax_class_" + this.count, 'value': tax_class});
        ...
        cell.appendChild(textEl);
        cell.width = 55;
        var textEl = this.createElement('input', {'type': 'text', 'size': 4, 'name': "quantity[" + this.count + "]", 'id': "quantity_" + this.count, 'value': the_quantity});
        ...
        var textEl = this.createElement('input', {'type': 'text', 'size': 30, 'name': "product_name[" + this.count + "]", 'id': itemName, 'value': product_name}, ["sqsEnabled", "sqsNoAutofill"]);
        textEl.count = this.count;
        textEl.alt = function() {
        ...
        var textEl = this.createElement('textarea', {'rows': 3, 'cols': 30, 'name': "product_description[" + this.count + "]", 'id': itemName, 'value': product_description.replace(/<br>/g, '\n')});
        textEl.count = this.count;
        textEl.alt = function() {
            quotesManager.lookup_item(itemName, document);
        }
        ...
        var buttonEl = this.createElement('button', {'type': 'button', 'title': this.selectButtonTitle, 'name': "btn_product_name[" + this.count + "]", 'id': itemName, 'value': this.selectButtonValue}, ['button']);
        ...
        var textEl = this.createElement('input', {'type': 'text', 'size': 10, 'name': "mft_part_num[" + this.count + "]", 'id': itemName, 'value': mft_part_num});
        item_list_MSI[itemName] = textEl;
        cell2.appendChild(textEl);
        var itemName = 'pricing_factor_' + this.count;
        var textEl = this.createElement('input', {'type': 'hidden', 'size': 4, 'name': "pricing_factor[" + this.count + "]", 'id': itemName, 'value': pricing_factor});
        item_list_MSI[itemName] = textEl;
        cell2.appendChild(textEl);
        var divselect = this.createElement('div', {'id': 'taxselect' + this.count});
        ...
        var selectEl = this.createElement('select', {'name': "tax_class_select_name[" + this.count + "]", 'id': itemName});
        ...
        var divnoselect = this.createElement('div', {'id': 'taxinput' + this.count});
        Y.one(divnoselect).hide();
        item_list_MSI['taxinput' + this.count] = divselect;
        var itemName = 'tax_class_name_' + this.count;
        var textEl = this.createElement('input', {'type': 'text', 'size': 8, 'name': 'tax_class_name[' + this.count + ']', 'id': itemName, 'value': tax_class_name});
        item_list_MSI[itemName] = textEl;
        ...
        var textEl = this.createElement('input', {'type': 'text', 'size': 8, 'name': "cost_price[" + this.count + "]", 'id': itemName, 'value': cost_price});
        ...
        var textEl = this.createElement('input', {'type': 'text', 'size': 8, 'name': "list_price[" + this.count + "]", 'id': itemName, 'value': list_price});
        textEl.style.textAlign = 'right';
        textEl.onchange = function() {
            if (!quotesManager.isAmount(toDecimal(unformatNumber(this.value, num_grp_sep, dec_sep), quotesManager.precision))) {
                alert(quotesManager.invalidAmount);
        ...
        var textEl = this.createElement('input', {'type': 'text', 'size': 8, 'name': "discount_price[" + this.count + "]", 'id': itemName, 'value': unit_price});
        textEl.style.textAlign = 'right';
        textEl.onchange = function() {
            if (quotesManager.isAmount(toDecimal(unformatNumber(this.value, num_grp_sep, dec_sep), quotesManager.precision))) {
                quotesManager.calculate(document);
        ...
        cell7.width = 60;
        var divselect = this.createElement('div', {'id': 'discount_amount_div' + this.count});
        ...
        var textEl = this.createElement('input', {'type': 'text', 'size': 4, 'name': "discount_amount[" + this.count + "]", 'id': itemName, 'value': discount_amount});
        textEl.style.textAlign = 'right';
        textEl.count = this.count;
        textEl.onchange = function() {
            if (quotesManager.isAmount(toDecimal(unformatNumber(this.value, num_grp_sep, dec_sep), quotesManager.precision))) {
        ...
        item_list_MSI[itemName] = textEl;
        var divnoselect = this.createElement('div', {'id': 'deal_calc' + this.count});
        Y.one(divnoselect).hide();
        item_list_MSI['deal_calc' + this.count] = divselect;
        var itemName = 'discount_select_' + this.count;
        var inputselect = this.createElement('input', {'type': 'hidden', 'name': "discount_select[" + this.count + "]", 'id': itemName, 'value': discount_select});
        divnoselect.appendChild(inputselect);
        item_list_MSI[itemName] = inputselect;
        var itemName = 'deal_calc_' + this.count;
        var textEl = this.createElement('input', {'type': 'text', 'size': 8, 'name': 'deal_calc[' + this.count + ']', 'id': itemName, 'value': deal_calc});
        item_list_MSI[itemName] = textEl;
        divnoselect.appendChild(textEl);        
        ...
        var buttonEl = this.createElement('input', {'type': 'button', 'name': this.deleteButtonName, 'id': 'delete_row' + this.count, 'value': this.deleteButtonValue}, ['button']);
        buttonEl.tableId = table_id;
        buttonEl.count = this.count;
        buttonEl.onclick = function() {
    ...
    this.calculate = function(doc) {
        ...
        for (var table_count = 0; table_count < this.table_list.length; table_count++) {
            ...
            if (table != null && typeof (table) != 'undefined') {
                ...
                if (retval['shipping'] != '')
                    gt['shipping'] += parseFloat(retval['shipping']);
            }
        }
    ...
    this.walk_the_kids = function(doc, children, variables, variable_values) {
        for (k = 0; k < children.length; k++) {
            child = children[k];
            if (child.nodeType == 1 && this.hasAttribute(child, 'id') && child.getAttribute('id') != child.getAttribute('name') && child.tagName != 'LI' && (child.tagName != 'DIV' || child.style.display != 'NONE')) {
